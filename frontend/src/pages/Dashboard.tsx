...
function ProgressBar({ label, value, max, unit, isDark, colorType = 'usage' }: ProgressBarProps) {
    ...
    >
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          <span style={{ fontSize: 12, color: isDark ? 'rgba(255, 255, 255, 0.5)' : '#64748b' }}>
            VRAM
          </span>
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        <div>
          <div style={{ fontSize: 11, color: isDark ? 'rgba(255, 255, 255, 0.4)' : '#64748b', marginBottom: 2 }}>
            Utilization
          </div>
          <div style={{ fontSize: 18, fontWeight: 600, color: utilizationColor }}>
            {gpu.utilization || 0}%
          </div>
        </div>
        <div>
          ...
          <div style={{ fontSize: 18, fontWeight: 600, color: temperatureColor }}>
            {gpu.temperature || 0}°C
          </div>
        </div>
      </div>
...
function WorkerCard({ worker, isDark }: WorkerCardProps) {
  ...
  const totalVRAM = worker.gpu_info?.reduce((sum, gpu) => sum + gpu.memory_total, 0) || 0
...
function CustomTooltip({ active, payload, label, isDark, formatter }: CustomTooltipProps) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 11, color: isDark ? 'rgba(255, 255, 255, 0.5)' : '#64748b', marginBottom: 4 }}>
        {label}
      </div>
...
export default function Dashboard() {
  ...
  // Chart data
  const requestChartData = usage.request_history?.map(point => ({
    date: point.date,
    value: point.value,
  })) || []

  const tokenChartData = usage.token_history?.map(point => ({
    date: point.date,
    value: point.value,
  })) || []
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      {/* GPU Overview */}
      <div style={{ marginTop: 32 }}>
        <SectionHeader title="GPU Overview" count={allGPUs.length} isDark={isDark} />
        ...
        <Row gutter={[16, 16]}>
          {allGPUs.map(({ gpu, workerName }, idx) => (
            <Col xs={24} sm={12} lg={8} xl={6} key={`${workerName}-${gpu.index}-${idx}`}>
      ...
      {/* Active Deployments */}
      <div style={{ marginTop: 32 }}>
        <SectionHeader title="Active Deployments" count={resources.deployment_running_count} isDark={isDark} />
        <Row gutter={[16, 16]}>
              ...
              {tokenChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={tokenChartData}>
                    ...
                    />
                    <RechartsTooltip
                      content={<CustomTooltip isDark={isDark} formatter={(v) => `${v.toLocaleString()} tokens`} />}
                    />
                    <Area
                      type="monotone"
                      dataKey="value"
