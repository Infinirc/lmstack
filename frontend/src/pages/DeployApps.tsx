...
import type { AppDefinition, DeployedApp, DeployProgress } from '../services/api'
...
const statusConfig: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
  ...
  pulling: { color: 'processing', icon: <CloudDownloadOutlined />, label: 'Pulling Image' },
  starting: { color: 'processing', icon: <LoadingOutlined />, label: 'Starting' },
  stopping: { color: 'warning', icon: <SyncOutlined spin />, label: 'Stopping' },
...
export default function DeployApps() {
  ...
  const [progressMap, setProgressMap] = useState<Record<number, DeployProgress>>({})
  ...
  useEffect(() => {
    const deployingApps = deployedApps.filter(
      app => ['pending', 'pulling', 'starting'].includes(app.status)
    )
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <>
          ...
          <Row gutter={[16, 16]}>
            {deployedApps.map((app) => {
              ...
              const isDeploying = ['pending', 'pulling', 'starting'].includes(app.status)
                          ...
                          {appLogos[app.app_type] && (
                            <img
                              ...
                              style={{
                                width: 32,
                                height: 32,
                                borderRadius: app.app_type === 'flowise' ? 0 : 6,
                                objectFit: 'contain',
                              }}
                            />
                          )}
                      ...
                      {isDeploying && (
                        <div style={{ marginTop: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                            {app.status === 'pulling' && <CloudDownloadOutlined />}
                            {app.status === 'starting' && <LoadingOutlined />}
                            {app.status === 'pending' && <SyncOutlined spin />}
                            <Text type="secondary" style={{ fontSize: 13 }}>
                              {progress?.stage === 'unknown' || !progress?.message
                                ? (app.status === 'starting'
                          ...
                          {(!progress || progress.stage === 'unknown' || (app.status === 'pulling' && progress.progress === 0)) ? (
                            <Progress
                              percent={100}
                              size="small"
                              status="active"
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <Text type="secondary">
            Select a worker to deploy {selectedApp?.name}. An API key will be automatically created.
          </Text>
        </div>
        ...
        {workers.length === 0 && (
          <div style={{ marginTop: 16 }}>
            <Text type="danger">No online workers available. Please add a worker first.</Text>
          </div>
        )}
      </Modal>
