...
import type { Container, ContainerState, Worker, ContainerImage } from '../types'
...
export default function Containers() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await containersApi.getLogs(logsModal.id, logsModal.worker_id, { tail: 500 })
  ...
  const handleRestart = async (container: Container) => {
    try {
      ...
      const err = error as { response?: { data?: { detail?: string } } }
      message.error(err.response?.data?.detail || 'Failed to restart container')
    }
  }
    ...
    try {
      const ports = values.ports
        ? values.ports.split(',').map((p) => {
            ...
            return { container_port: parseInt(container || host), host_port: parseInt(host) }
          })
        : undefined
  ...
  const handleViewLogs = async (container: Container) => {
    ...
    setLogsLoading(true)
    try {
      const response = await containersApi.getLogs(container.id, container.worker_id, { tail: 500 })
      setLogs(response.logs)
    } catch {
  ...
  const handleExec = async () => {
    ...
    setExecLoading(true)
    try {
      const result = await containersApi.exec(execModal.id, execModal.worker_id, {
      ...
      const err = error as { response?: { data?: { detail?: string } } }
      setExecResult(`Error: ${err.response?.data?.detail || 'Failed to execute command'}`)
    } finally {
      ...
      render: (_: unknown, record: Container) => (
        <div>
          ...
          <div style={{ marginTop: 4 }}>
            <Tag color={STATE_COLORS[record.state]}>{record.state.toUpperCase()}</Tag>
          </div>
          ...
          {record.deployment_name && (
            <div style={{ fontSize: 11, color: '#888' }}>Deployment: {record.deployment_name}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (_: unknown, record: Container) =>
        record.ports.length > 0
          ? record.ports.map((p) => `${p.host_port}:${p.container_port}`).join(', ')
          : '-',
    },
    {
      title: 'Actions',
        ...
        <Space>
          ...
          {record.state === 'running' && (
            <Tooltip title="Terminal">
              <Button type="text" icon={<CodeOutlined />} onClick={() => setExecModal(record)} />
            </Tooltip>
          )}
          <Popconfirm
            title="Delete container?"
    ...
    <div>
      ...
      >
        <Form form={createForm} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="command" label="Command" extra="Optional command to run">
            ...
            items={[
              {
                ...
                children: (
                  <Descriptions column={isMobile ? 1 : 2} bordered size="small">
                    ...
                    <Descriptions.Item label="Image">{detailModal.image}</Descriptions.Item>
                    <Descriptions.Item label="Worker">{detailModal.worker_name}</Descriptions.Item>
                    <Descriptions.Item label="Status">{detailModal.status}</Descriptions.Item>
                ...
                children: (
                  ...
                  >
                    {detailModal.env && detailModal.env.length > 0 ? (
                      ...
                      <span style={{ color: '#888' }}>No environment variables</span>
                    )}
                  </div>
                ),
              },
