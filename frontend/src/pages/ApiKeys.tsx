...
import { apiKeysApi, deploymentsApi, modelsApi, type ApiKeyStats } from '../services/api'
...
function CodeExamples({ apiKey, baseUrl, model = 'your-model-name' }: CodeExamplesProps) {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <Text style={{ color: isDark ? '#71717a' : '#64748b', fontSize: 13, fontWeight: 500 }}>{title}</Text>
          <div style={{ marginTop: 8 }}>
            ...
            {suffix && (
              <Text style={{ color: isDark ? '#71717a' : '#64748b', fontSize: 14, marginLeft: 4 }}>{suffix}</Text>
            )}
          </div>
        </div>
        <div style={{
...
export default function ApiKeys() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setRunningDeployments(deploymentsRes.items.filter(d => d.status === 'running'))
      setStats(statsRes)
    } catch (error) {
          ...
          <div>
            ...
            {keyStats && (
              <div style={{ fontSize: 11, color: '#888', marginTop: 4 }}>
                {keyStats.requests.toLocaleString()} req · {keyStats.tokens.toLocaleString()} tokens
              </div>
            )}
          </div>
        )
  ...
  const desktopColumns: ColumnsType<ApiKey> = [
          ...
          <Space>
            ...
            <Tooltip title="Copy key prefix">
              <Button
                ...
                size="small"
                icon={<CopyOutlined />}
                onClick={() => copyToClipboard(`lmsk_${accessKey}`, 'Key prefix copied')}
              />
            </Tooltip>
          </Space>
        )
    ...
    },
    {
      ...
      render: (_: unknown, record: ApiKey) => {
        ...
        return (
          <div>
            <div><Text style={{ fontSize: 13 }}>{(keyStats?.requests || 0).toLocaleString()} requests</Text></div>
            <div>
              <Text
                style={{ fontSize: 12 }}
  ...
  const defaultModel = runningDeployments.length > 0
    ? (runningDeployments[0].model?.name || runningDeployments[0].name)
    : (availableModels.length > 0 ? availableModels[0].name : 'your-model-name')
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <StatsCard
            title="Active Keys"
            value={apiKeys.filter(k => !k.expires_at || dayjs(k.expires_at).isAfter(dayjs())).length}
        ...
        >
          ...
          <div style={{ padding: 24 }}>
            <Text type="secondary" style={{ display: 'block', marginBottom: 16 }}>
              Use the OpenAI SDK with your LMStack endpoint. Base URL: <code>{baseUrl}/v1</code>
            </Text>
            <Tabs
              items={[
                {
  ...
  -d '{"model": "${defaultModel}", "messages": [{"role": "user", "content": "Hello"}]}'`}
      ...
      >
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>Create API Key</h2>
        ...
        <Form form={createForm} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item
            ...
            label="Model Access"
            extra={availableModels.length > 0
          ...
          >
            <Select
              mode="multiple"
              placeholder={availableModels.length > 0 ? "All running models" : "No running models"}
      ...
      <Modal
        ...
        footer={
          <Button type="primary" size={isMobile ? 'middle' : 'large'} block onClick={() => setNewKeyModal(null)}>
            Done
          </Button>
        }
            ...
            <div style={{
              ...
              <div style={{
                ...
                <Button
                  ...
                  size="small"
                  icon={<CopyOutlined />}
                  onClick={() => copyToClipboard(newKeyModal.key, 'API key copied')}
                >
                  Copy
                </Button>
              </div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <Button
                  ...
                  size="small"
                  icon={<CopyOutlined />}
                  onClick={() => copyToClipboard(`${baseUrl}/v1`, 'Base URL copied')}
                />
              </div>
            </div>
