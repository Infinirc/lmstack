...
export default function Deployments() {
  ...
  const selectedGpuMemoryGb = (() => {
    ...
    const gpuIndexesToCheck =
      selectedGpuIndexes.length > 0 ? selectedGpuIndexes : [0];
  ...
  const editingBackend = (editingDeployment?.backend || "vllm") as
    | "vllm"
    | "sglang"
    | "ollama";
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const err = error as { response?: { data?: { detail?: string } } };
      message.error(
        err.response?.data?.detail || "Failed to delete deployment",
      );
    }
  };
  ...
  const handleUpdateDeployment = async (values: {
  ...
  const renderSourceTag = (
    ...
    return (
      <Tag style={getTagStyle(size)}>
        {isOllama ? (
          <OllamaLogo height={iconHeight} isDark={isDark} />
        ) : (
          <HuggingFaceLogo height={iconHeight} />
        )}
      ...
      >
        {editingDeployment && (
          <Form
            form={editForm}
            layout="vertical"
            onFinish={handleUpdateDeployment}
          >
            {/* Deployment Info */}
            <div
              style={{
              ...
              >
                <div>
                  ...
                  <div style={{ fontWeight: 500 }}>
                    {editingDeployment.model?.name}
                  </div>
                </div>
                <Tag color={getStatusColor(editingDeployment.status)}>
                  {editingDeployment.status.toUpperCase()}
                </Tag>
              </div>
              <div style={{ fontSize: 12, color: "#888" }}>
                Worker: {editingDeployment.worker?.name}
              </div>
            </div>
