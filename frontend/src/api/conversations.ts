...
export const conversationsApi = {
  list: async (params?: ConversationListParams): Promise<ListResponse<Conversation>> => {
    const response = await api.get<ListResponse<Conversation>>('/conversations', { params })
  ...
  addMessages: async (id: number, data: AddMessagesData): Promise<Conversation> => {
    const response = await api.post<Conversation>(`/conversations/${id}/messages`, data)
    return response.data
  },
