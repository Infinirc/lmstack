...
export const ollamaApi = {
  ...
  getModelInfo: async (modelName: string): Promise<OllamaModel> => {
    const response = await api.get<OllamaModel>(`/ollama/model/${encodeURIComponent(modelName)}`)
    return response.data
  },

  getModelTags: async (modelName: string): Promise<{ model: string; tags: OllamaTagInfo[] }> => {
