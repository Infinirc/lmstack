...
export const huggingfaceApi = {
  getModelInfo: async (modelId: string, token?: string): Promise<HFModelInfo> => {
  ...
  listFiles: async (modelId: string, token?: string): Promise<HFModelFile[]> => {
    const params: Record<string, string> = {}
    if (token) params.token = token
  ...
  getReadme: async (modelId: string, token?: string): Promise<{ content: string | null; message?: string }> => {
    const params: Record<string, string> = {}
    if (token) params.token = token
    const response = await api.get<{ content: string | null; message?: string }>(
      `/huggingface/readme/${encodeURIComponent(modelId)}`,
      { params }
    )
    return response.data
  },
