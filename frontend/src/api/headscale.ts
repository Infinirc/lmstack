...
export const headscaleApi = {
  ...
  start: async (data?: {
    ...
    http_port?: number;
    grpc_port?: number;
  }): Promise<HeadscaleStatus> => {
    const response = await api.post<HeadscaleStatus>(
      "/headscale/start",
      data || {},
    );
    return response.data;
  ...
  createPreauthKey: async (data?: {
    ...
    const response = await api.post<PreauthKeyResponse>(
  ...
  listNodes: async (): Promise<{ items: HeadscaleNode[]; total: number }> => {
    const response = await api.get<{ items: HeadscaleNode[]; total: number }>(
      "/headscale/nodes",
    );
    return response.data;
  },
