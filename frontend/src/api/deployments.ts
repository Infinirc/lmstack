...
export const deploymentsApi = {
  list: async (params?: DeploymentListParams): Promise<ListResponse<Deployment>> => {
    const response = await api.get<ListResponse<Deployment>>('/deployments', { params })
  ...
  getLogs: async (id: number, tail: number = 100): Promise<DeploymentLogsResponse> => {
    const response = await api.get<DeploymentLogsResponse>(`/deployments/${id}/logs`, {
      params: { tail },
    })
    return response.data
  },

  update: async (id: number, data: Partial<DeploymentCreate>): Promise<Deployment> => {
