...
export const modelFilesApi = {
  list: async (params?: ModelFileListParams): Promise<ListResponse<ModelFileView>> => {
    const response = await api.get<ListResponse<ModelFileView>>('/model-files', { params })
    return response.data
  },

  delete: async (modelId: number, workerId: number): Promise<ModelFileDeleteResponse> => {
