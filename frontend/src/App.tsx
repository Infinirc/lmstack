...
function getMenuItems(isAdmin: boolean) {
  const workersChildren: any[] = [
    { key: '/workers', icon: <CloudServerOutlined />, label: 'Overview' },
    {
      ...
      children: [
        ...
        { key: '/containers', icon: <CodeSandboxOutlined />, label: 'Containers' },
  ...
  if (isAdmin) {
    workersChildren.push({ key: '/headscale', icon: <GlobalOutlined />, label: 'Headscale VPN' })
  }
  ...
  const items: any[] = [
    ...
    { key: '/models', icon: <ExperimentOutlined />, label: 'Models', style: { marginTop: 16 } },
    { key: '/deployments', icon: <DeploymentUnitOutlined />, label: 'Deploy Model' },
    { key: '/deploy-apps', icon: <RocketOutlined />, label: 'Deploy Apps' },
    { key: '/api-keys', icon: <ApiOutlined />, label: 'API Gateway', style: { marginTop: 16 } },
  ]
  ...
  for (const item of menuItems) {
    ...
    if (item.children) {
      for (const child of item.children) {
        ...
        if (child.children) {
          const subChild = child.children.find((c: { key: string }) => c.key === pathname)
...
function AppRoutes() {
  ...
  if (isInitialized && !user && !['/login', '/setup'].includes(location.pathname)) {
    return <Navigate to="/login" state={{ from: location }} replace />
  }
