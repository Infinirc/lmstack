...
import { huggingfaceApi, type HFModelInfo, type VRAMEstimate, type HFSearchResult } from '../services/api'
...
export default function HuggingFaceModelPicker({
  ...
  const searchModels = useCallback(async (query: string) => {
    ...
    try {
      ...
      setTotalResults(results.length >= pageSize ? pageSize * 50 : results.length)
    } catch (error) {
      ...
      footer={
        <div style={{
          ...
          <Button
            ...
            disabled={!selectedModelId || !!(vramEstimate && !vramEstimate.compatible)}
    ...
    >
      <div style={{ display: 'flex', width: '100%', height: '100%', flexDirection: isMobile ? 'column' : 'row' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            {searching ? (
              <div style={{ textAlign: 'center', padding: 40 }}>
                <Loading />
                <div style={{ marginTop: 8, color: colors.textMuted }}>Searching...</div>
              </div>
                ...
                renderItem={(item) => (
                  <List.Item
                    ...
                    style={{
                      padding: '12px 16px',
                      cursor: 'pointer',
                      background: selectedModelId === item.id
                        ? (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.04)')
                        : 'transparent',
                      borderLeft: selectedModelId === item.id
                        ? '3px solid #1890ff'
                      ...
                      {item.tags && item.tags.length > 0 && (
                        <div style={{ marginTop: 4 }}>
                          {item.tags.slice(0, 3).map(tag => (
                            <Tag key={tag} style={{ fontSize: 10, marginRight: 4 }}>
                              {tag}
                            </Tag>
                          ))}
                        </div>
        ...
        <div style={{
          ...
          {!selectedModelId ? (
            <div style={{ textAlign: 'center', padding: isMobile ? 40 : 60, color: colors.textMuted }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              {/* Scrollable Content */}
              <div style={{ flex: 1, overflow: 'auto', padding: 16, paddingTop: 8, paddingBottom: 32 }}>
                {/* Model ID Display */}
                <div
                  style={{
                    padding: 16,
                  ...
                  >
                    ...
                    <Descriptions size="small" column={2}>
                      ...
                      <Descriptions.Item label="Precision">
                        <Tag color="cyan">{vramEstimate.precision.toUpperCase()}</Tag>
                      </Descriptions.Item>
                ...
                {readme ? (
                  ...
                  <div style={{ padding: '16px 0', textAlign: 'center', marginBottom: 24 }}>
                    <Text type="secondary">
                      README not available (This may be a gated model requiring HuggingFace authentication)
                    </Text>
                    <br />
                    <Button
                      type="link"
