...
import { ollamaApi, type OllamaModel, type OllamaTagInfo } from '../services/api'
...
export default function OllamaModelPicker({
  ...
  // Search models
  const searchModels = useCallback(async (query: string, capability?: string | null) => {
  ...
  const handleSelectModel = useCallback(async (model: OllamaModel) => {
    ...
    try {
      ...
      if (result.tags.length > 0) {
        const latestTag = result.tags.find(t => t.name === 'latest') || result.tags[0]
      ...
      if (fallbackTags.length === 0) {
        fallbackTags.push({ name: 'latest', full_name: `${model.name}:latest` })
      }
    ...
    try {
      ...
      setSelectedModel(prev => prev ? { ...prev, readme: modelInfo.readme } : prev)
    } catch (error) {
  ...
  const handleConfirmSelect = () => {
    if (selectedModel) {
      const fullName = selectedTag ? `${selectedModel.name}:${selectedTag}` : selectedModel.name
  ...
  return (
    <Drawer
      title={isMobile && showDetails ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Button type="text" size="small" onClick={() => setShowDetails(false)}>
            ← Back
          </Button>
      ...
      footer={
        <div style={{
          ...
          <Button size={isMobile ? 'middle' : 'large'} onClick={onClose}>Cancel</Button>
          <Button
            type="primary"
    ...
    >
      <div style={{ display: 'flex', width: '100%', height: '100%', flexDirection: isMobile ? 'column' : 'row' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            {searching ? (
              <div style={{ textAlign: 'center', padding: 40 }}>
                <Loading />
                <div style={{ marginTop: 8, color: colors.textMuted }}>Loading models...</div>
              </div>
            ) : models.length === 0 ? (
              <Empty
                ...
                renderItem={(item) => (
                  <List.Item
                    ...
                    style={{
                      padding: '12px 16px',
                      cursor: 'pointer',
                      background: selectedModel?.name === item.name
                        ? (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.04)')
                        : 'transparent',
                      borderLeft: selectedModel?.name === item.name
                        ? '3px solid #1890ff'
          ...
          {!selectedModel ? (
            ...
            <div style={{ padding: 20 }}>
              ...
              <div>
                ...
                {loadingTags ? (
                  <div style={{ textAlign: 'center', padding: 20 }}>
                    <Loading size="small" />
                    <Text type="secondary" style={{ marginLeft: 8 }}>Loading available tags...</Text>
                  </div>
                ) : (
                  <div>
                    <Select
                      ...
                      options={tags.map(tag => ({
                        label: (
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span>{tag.name}</span>
                            <Space size={4}>
                              {tag.size && <Tag color="blue" style={{ margin: 0 }}>{tag.size}</Tag>}
                              {tag.quantization && <Tag style={{ margin: 0 }}>{tag.quantization}</Tag>}
                            </Space>
                          </div>
                        ),
                        value: tag.name,
                    ...
                    >
                      <Text type="secondary" style={{ fontSize: 12, marginBottom: 4, display: 'block' }}>Model ID:</Text>
                      <Text code style={{ fontSize: 16, fontWeight: 600 }}>
                        {selectedModel.name}{selectedTag ? `:${selectedTag}` : ''}
                      </Text>
                    </div>
              ...
              <div>
                ...
                >
                  ollama run {selectedModel.name}{selectedTag && selectedTag !== 'latest' ? `:${selectedTag}` : ''}
                </div>
              </div>
              ...
              {loadingReadme ? (
                <div style={{ textAlign: 'center', padding: 20 }}>
                  <Loading size="small" />
                  <Text type="secondary" style={{ marginLeft: 8 }}>Loading README...</Text>
                </div>
                ...
                >
                  <ReactMarkdown
                    ...
                    components={{
                      h1: ({ children }) => <h1 style={{ fontSize: '1.5em', fontWeight: 600, marginTop: 24, marginBottom: 16, borderBottom: `1px solid ${colors.border}`, paddingBottom: 8 }}>{children}</h1>,
                      h2: ({ children }) => <h2 style={{ fontSize: '1.3em', fontWeight: 600, marginTop: 24, marginBottom: 16, borderBottom: `1px solid ${colors.border}`, paddingBottom: 8 }}>{children}</h2>,
                      h3: ({ children }) => <h3 style={{ fontSize: '1.1em', fontWeight: 600, marginTop: 24, marginBottom: 16 }}>{children}</h3>,
                      p: ({ children }) => <p style={{ marginTop: 0, marginBottom: 16 }}>{children}</p>,
                      ul: ({ children }) => <ul style={{ paddingLeft: 24, marginTop: 0, marginBottom: 16 }}>{children}</ul>,
                      ol: ({ children }) => <ol style={{ paddingLeft: 24, marginTop: 0, marginBottom: 16 }}>{children}</ol>,
                      li: ({ children }) => <li style={{ marginBottom: 4 }}>{children}</li>,
                      code: ({ className, children }) => {
                        const isBlock = className?.includes('language-')
                        return isBlock ? (
                          <pre style={{ background: isDark ? '#1e1e1e' : '#f6f8fa', padding: 16, borderRadius: 6, overflow: 'auto', marginBottom: 16 }}>
                            <code style={{ fontSize: 13 }}>{children}</code>
                          </pre>
                        ) : (
                          <code style={{ background: isDark ? '#1e1e1e' : '#f6f8fa', padding: '2px 6px', borderRadius: 4, fontSize: 13 }}>{children}</code>
                        )
                      },
                      pre: ({ children }) => <>{children}</>,
                      a: ({ href, children }) => <a href={href} target="_blank" rel="noopener noreferrer" style={{ color: '#1890ff' }}>{children}</a>,
                      img: ({ src, alt }) => <img src={src} alt={alt || ''} style={{ maxWidth: '100%', height: 'auto', borderRadius: 4, marginBottom: 16 }} />,
                      blockquote: ({ children }) => <blockquote style={{ margin: '0 0 16px', padding: '0 16px', borderLeft: `4px solid ${colors.border}`, color: colors.textMuted }}>{children}</blockquote>,
                      table: ({ children }) => <div style={{ overflowX: 'auto', marginBottom: 16 }}><table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 400 }}>{children}</table></div>,
                      thead: ({ children }) => <thead>{children}</thead>,
                      tbody: ({ children }) => <tbody>{children}</tbody>,
                      tr: ({ children }) => <tr>{children}</tr>,
                      th: ({ children }) => <th style={{ border: `1px solid ${colors.border}`, padding: '8px 12px', background: isDark ? '#1e1e1e' : '#f6f8fa', fontWeight: 600, textAlign: 'left', whiteSpace: 'nowrap' }}>{children}</th>,
