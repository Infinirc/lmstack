...
curl -X POST http://localhost:52000/api/api-keys \
...
```bash
curl -X POST http://localhost:52000/api/deployments \
...
```bash
curl http://localhost:52000/api/deployments/DEPLOYMENT_ID \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
client = OpenAI(
    ...
    base_url="http://localhost:52000/v1"
)
...
```bash
curl http://localhost:52000/api/deployments/DEPLOYMENT_ID/logs \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl http://localhost:52000/api/workers/WORKER_ID \
  -H "Authorization: Bearer YOUR_API_KEY"
```
